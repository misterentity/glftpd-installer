# glFTPd Installer GUI

A Windows GUI application for installing glFTPd on remote Linux servers via SSH.

## Features

- SSH connection to remote Linux server
- GUI interface for all installation options
- Real-time installation log
- Secure password handling
- Remote script execution with sudo privileges
...
```bash
git clone https://github.com/misterentity/glftpd-installer.git
cd glftpd-installer
```
...
1. **SSH Connection Tab**
   - Enter the SSH host address
   - Enter the SSH port (default: 22)
   - Enter your SSH username
   - Enter your SSH password
   - Click "Connect" to establish SSH connection

2. **Installation Options Tab**
   - Fill in all required installation options
   - Default values are provided for common settings
   - Scroll through the options to see all available settings
   - Click "Start Installation" to begin the installation process

3. **Installation Log Tab**
   - View real-time installation progress
   - Monitor any errors or warnings
   - Installation output is displayed here

## Security Notes

- SSH passwords are handled securely and not stored
- Sudo password is only used during installation
...
1. **Connection Issues**
   ...
   - Check if SSH port is open and accessible
   - Ensure target server is running and accessible

2. **Installation Issues**
   - Check the installation log for error messages
   - Verify sudo privileges on target server
   - Ensure all required packages are installed on target server
